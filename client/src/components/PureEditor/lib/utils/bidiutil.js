 ...
 **/
bidiUtil.doBidiReorder = function (text, textCharTypes, isRtl) {
	if (text.length < 2)
		return {};
	...
	for (var i = 0; i < logicalFromVisual.length - 1; i++) { //fix levels to reflect character width
		if (textCharTypes[i] === AN) {
			levels[i] = bidiUtil.AN;
		} else if (levels[i] === R && ((textCharTypes[i] > AL && textCharTypes[i] < LRE)
			|| textCharTypes[i] === ON || textCharTypes[i] === BN)) {
			levels[i] = bidiUtil.ON_R;
		} else if ((i > 0 && chars[i - 1] === '\u0644') && /\u0622|\u0623|\u0625|\u0627/.test(chars[i])) {
			levels[i - 1] = levels[i] = bidiUtil.R_H;
			i++;
		}
	}
	/* fix level to mark zero length EOL */
	if (chars[chars.length - 1] === bidiUtil.DOT)
		levels[chars.length - 1] = bidiUtil.B;

	if (chars[0] === '\u202B')
		levels[0] = bidiUtil.RLE;
 ...
 **/
bidiUtil.hasBidiCharacters = function (text, textCharTypes) {
	var ret = false;
 ...
 **/
bidiUtil.getVisualFromLogicalIdx = function (logIdx, rowMap) {
